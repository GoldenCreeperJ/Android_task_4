...
class DiaryListAdapter(private val diaryViewModel: DiaryViewModel):RecyclerView.Adapter<DiaryListAdapter.ViewHolder>() {
    ...
    inner class ViewHolder(val binding: DiaryListItemBinding): RecyclerView.ViewHolder(binding.root){
        init { this.itemView.setOnClickListener {clickListener?.onClick(adapterPosition)}}}
